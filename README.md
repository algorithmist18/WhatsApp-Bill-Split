...
2. **Upload a bill** — hit 📎 / **Split a bill**, drop in a photo of a receipt.
   A **mock OCR** step "reads" the items and amounts (editable afterwards).
3. **Split it** — two ways:
...
State is saved in `localStorage`, so your group and splits survive a refresh.
...
- **OCR is mocked** — it returns a realistic itemised receipt rather than reading
  the actual pixels. Swap `src/lib/ocr.js` for Tesseract.js / a vision API to make
  it real; the rest of the flow already works on the returned items.
...
- One payer per bill: everyone else owes the payer their share.
...
src/
  ...
  lib/
    ocr.js                mock receipt OCR
    split.js              share + debt calculation
